...
class TuneRepository extends DBManager
{
	...
	public function addTune(Tune $tune) {
		...
		$this->query("INSERT INTO tune (fk_user_tune, title_tune, composer, category_tune, date_tune) VALUES (?, ?, ?, ?, ?);",array($iduser, $title, $composer, $category, $datetune));
	...
	public function FindUserLikedtune($iduser) {
		...
		foreach ($tunedata as $tuple) {
			$tunelist[] =  new Tune($tuple['id_tune'],$tuple['fk_user_tune'],$tuple['title_tune'],$tuple['composer'],$tuple['category_tune'],$tuple['date_tune']);
		}
		return $tunelist;
	}
	...
	public function FindUserLikedtuneGroupByCategory($iduser) {
		...
		$tunelist = array();
		foreach ($tunedata as $tuple) {
			$tunelist[$tuple['category_tune']][] =  new Tune($tuple['id_tune'],$tuple['fk_user_tune'],$tuple['title_tune'],$tuple['composer'],$tuple['category_tune'],$tuple['date_tune']);
		}
		return $tunelist;
	}
	...
	/*public function findTuneById($idtune) {
        ...
        public function findTuneById($idtune) {
		$tunedata = $this->query("SELECT * from tune WHERE id_tune = (?) limit 1;",array($idtune));
                $scoredata = $this->query("SELECT * from score WHERE fk_tune_score = (?);",array($idtune));
                if (!empty($tunedata)) {
                    $tuple = $tunedata[0];
                    if (!empty($scoredata)) {
                        $datescore = array();
                    ...
                    $tune =  new Tune($tuple['id_tune'],$tuple['fk_user_tune'],$tuple['title_tune'],$tuple['composer'],$tuple['category_tune'],$tuple['date_tune'],$pdfscore,$forkedusers,$datescore,$idscorelist);
                    return $tune;
                } else {
                    return false;
                }
	...
	public function findTuneByTitle($title) {
            ...
            if (!empty($tunedata)) {
                $tuple = $tunedata[0];
                $tune =  new Tune($tuple['id_tune'],$tuple['fk_user_tune'],$tuple['title_tune'],$tuple['composer'],$tuple['category_tune'],$tuple['date_tune']);
                return $tune;
            } else {
                return false;
            }
        ...
        public function deleteTuneForAll($idtune) {
                if ($this->query("DELETE from likedtune  WHERE fk_tune_lt = (?);",array($idtune)) != null) {
                    if ($this->query("DELETE from score WHERE fk_tune_score = (?);",array($idtune)) != null) {
