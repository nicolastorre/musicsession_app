...
var nbbeats, beatunit, draw, svgGraph, score, totalnote, totalbar, nstaff, noteduration, type, typeunit, currentBar, type;
...
function drawStaff(nstaff) {
	for(var i=0;i<5;i++) {
		var idline = "line_"+i;
		draw.line(0,200+(nstaff*200)+(i*10),1000,200+(nstaff*200)+(i*10)).stroke({ width: 2}).attr("id",idline);
	}	
}
...
function drawBarLine(x,nstaff) {
	draw.line(x,200+(nstaff*200),x,240+(nstaff*200)).stroke({ width: 2});
}

function drawNote(type ,mouseX ,mouseY,id ) {
	switch (type) {
		case "eighth":
			draw.circle(11).attr({ cx: mouseX, cy: mouseY }).attr("id",id);
			draw.line(mouseX + 5, mouseY, mouseX + 5, mouseY - 30).stroke({ width: 2 });
			draw.line(mouseX + 5, mouseY - 30, mouseX + 15, mouseY - 15).stroke({ width: 2 });
			break;
		case "quarter":
			draw.circle(11).attr({ cx: mouseX, cy: mouseY }).attr("id",id);
			draw.line(mouseX + 5, mouseY, mouseX + 5, mouseY - 30).stroke({ width: 2 });
			break;
		case "half":
			draw.circle(11).attr({ cx: mouseX, cy: mouseY }).fill({ color: '#f06', opacity: 0.0 }).stroke({color: '#000', width: 2}).attr("id",id);
			draw.line(mouseX + 5, mouseY, mouseX + 5, mouseY - 30).stroke({ width: 2 });
			break;
		case "whole":
			draw.circle(11).attr({ cx: mouseX, cy: mouseY }).fill({ color: '#f06', opacity: 0.0 }).stroke({color: '#000', width: 2}).attr("id",id);
...
function addNote(evt) {
	...
	if ( mouseY>=stafflimittop && mouseY<stafflimitbot) { //mouseX<mouseX_raw &&
		...
		drawNote(type, mouseX, mouseY, score.length);
		idnote = "note_"+totalnote;
		score[idnote] = {pos:newpos, step: refStep[n%7], octave: refOctave[n], duration: noteduration[type], type: type};
		totalnote++;
		currentBar += score[idnote].duration;

		//tmp
		var note = (score[idnote].step + score[idnote].octave).toUpperCase()
